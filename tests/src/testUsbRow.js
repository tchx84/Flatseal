...
describe('FlatsealUsbRow', function() {
    ...
    _handles('negated all devices', '!all');
    _handles('vendor id', 'vnd:0123');
    _handles('vendor id (uppercase hex)', 'vnd:ABCD');
    _handles('vendor id (mixed hex)', 'vnd:aAbB');
    _handles('device id', 'dev:1234');
    _handles('class id', 'cls:ff');
    _handles('class id (uppercase)', 'cls:AB');
    _handles('negated vendor id', '!vnd:0123');
    _handles('negated device id', '!dev:1234');
    _handles('negated class id', '!cls:ff');
    _handles('vendor + device compound', 'vnd:0123+dev:4567');
    _handles('vendor + class compound', 'vnd:0123+cls:ab');
    _handles('device + class compound', 'dev:0123+cls:ff');
    _handles('vendor + device + class compound', 'vnd:0123+dev:4567+cls:ff');
    _handles('negated vendor + device compound', '!vnd:0123+dev:4567');
    ...
    _catches('class id with 3 hex digits', 'cls:fff');
